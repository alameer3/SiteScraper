...
class DataManager:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with sqlite3.connect(cache_db_path) as conn:
                ...
                # الإدخالات النشطة
                cursor.execute("SELECT COUNT(*) FROM url_cache WHERE expires_at > ?", (datetime.now(),))
                active_entries = cursor.fetchone()[0]

                # الإدخالات المنتهية الصلاحية
                expired_entries = total_entries - active_entries

                # أكثر الـ URLs استخداماً
                cursor.execute("""
                    SELECT url, access_count as count 
                    FROM url_cache 
                    GROUP BY url 
                    ORDER BY count DESC 
                    LIMIT 10
    ...
    def save_ai_analysis(self, url: str, analysis: Dict[str, Any]) -> str:
        """حفظ تحليل الذكاء الاصطناعي"""
        ...
        try:
            with sqlite3.connect(cache_db_path) as conn:
                ...
                # إنشاء جدول التحليلات إذا لم يكن موجوداً
                cursor.execute("""
                ...
                # حفظ التحليل
                cursor.execute("""
                    INSERT INTO ai_analyses (id, url, analysis) 
                    VALUES (?, ?, ?)
                """, (analysis_id, url, json.dumps(analysis)))

                conn.commit()
                return analysis_id

        except Exception as e:
            self.logger.error(f"Error saving AI analysis: {e}")
            raise

    def get_ai_analysis(self, analysis_id: str) -> Optional[Dict[str, Any]]:
